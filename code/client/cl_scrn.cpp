...
void SCR_DrawNamedPic( float x, float y, float width, float height, const char *picname ) {
	...
	re.DrawStretchPic( x, y, width, height, 0, 0, 1, 1, hShader, RENDER2D_STRETCH ); //Fluffy (Widescreen2D)
}
...
void SCR_FillRect( float x, float y, float width, float height, const float *color ) {
	...
	re.DrawStretchPic( x, y, width, height, 0, 0, 0, 0, cls.whiteShader, RENDER2D_STRETCH); //Fluffy (Widescreen2D)
...
void SCR_DrawDebugGraph (void)
{
	...
	re.DrawStretchPic(x, y - cl_graphheight->integer, 
		w, cl_graphheight->integer, 0, 0, 0, 0, 0, RENDER2D_STRETCH); //Fluffy (Widescreen2D)
	re.SetColor( NULL );
	...
	for (a=0 ; a<w ; a++)
	{
		...
		re.DrawStretchPic( x+w-1-a, y - h, 1, h, 0, 0, 0, 0, 0, RENDER2D_STRETCH); //Fluffy (Widescreen2D)
	}
}
