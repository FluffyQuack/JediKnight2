...
void CG_DrawIconBackground(void)
{
	...
	if ((cg.iconSelectTime+WEAPON_SELECT_TIME)<cg.time)	// Time is up for the HUD to display
	{
		if (cg.iconHUDActive)		// The time is up, but we still need to move the prongs back to their original position
		{
			...
			CG_DrawPic( x2+60, y2+30, 460, -height, background, RENDER2D_STRETCH);	// Top half //Fluffy (Widescreen2D)
			CG_DrawPic( x2+60, y2+30-2,460, height, background, RENDER2D_STRETCH);	// Bottom half //Fluffy (Widescreen2D)
		...
		CG_DrawPic( prongLeftX+xAdd, y2-10, 40, 80, cgs.media.weaponProngsOff, RENDER2D_ANCHOR_LEFT); //Fluffy (Widescreen2D)
		CG_DrawPic( prongRightX-xAdd, y2-10, -40, 80, cgs.media.weaponProngsOff, RENDER2D_ANCHOR_RIGHT); //Fluffy (Widescreen2D)
		return;
	}
